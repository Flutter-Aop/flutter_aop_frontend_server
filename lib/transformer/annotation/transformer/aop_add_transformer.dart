...
class AopAddTransformer extends RecursiveVisitor<void> {
  AopAddTransformer(this.infoList, this.uriToSource);
  ...
  void visitClass(Class node) {
    ...
    List<AopItemInfo>? items = _filterAopItemInfo(
        infoList, procedureImportUri, node.name, node.superclass);
    if (items.isNotEmpty) {
  ...
  void insertMethod4Class(AopItemInfo aopItemInfo, Class pointCutClass) {
    final Procedure originProcedure =
        aopItemInfo.aopMember!.function!.parent as Procedure;
    ...
    final Map<String, String> sourceInfo =
        AopUtils.calcSourceInfo(uriToSource, _curLibrary!, 0);

    final FunctionNode originFunctionNode =
        aopItemInfo.aopMember!.function as FunctionNode;
