...
import 'package:aop_frontend_server/transformer/annotation/manager/library_manager.dart';
...
class AopUtils {
  ...
  static String kImportUriAopAspect =
      'package:flutter_aop/annotation/aspect.dart';
  static String kImportUriAopCall = 'package:flutter_aop/annotation/call.dart';
  static String kImportUriAopExecute =
      'package:flutter_aop/annotation/execute.dart';
  static String kImportUriAopInject =
      'package:flutter_aop/annotation/inject.dart';
  static String kImportUriAopAdd = 'package:flutter_aop/annotation/add.dart';
  static String kImportUriAopFieldInitializer =
      'package:flutter_aop/annotation/field_initializer.dart';
  static String kImportUriAopFieldGet =
      'package:flutter_aop/annotation/field_get.dart';
  static String kImportUriPointCut = 'package:flutter_aop/entity/pointcut.dart';
  static String kAopUniqueKeySeparator = '#';
  ...
  static String kAopAnnotationExcludeLib = 'excludeCoreLib';
  ...
  static String kPointCutClass = 'PointCut';
  static String kInstanceMethodPrefix = '-';
  static String kStaticMethodPrefix = '+';
  static int kPrimaryKeyAopMethod = 0;
  static String kAopStubMethodPrefix = 'aop_stub_';
  static String kProcessMethod = 'proceed';
  ...
  static AopMode? getAopMode(String name, String importUri) {
  ...
  static void concatArgumentsForAopMethod(
    ...
    final List<MapLiteralEntry> sourceList = <MapLiteralEntry>[];
    sourceInfo.forEach((String key, String value) {
      sourceList.add(MapLiteralEntry(StringLiteral(key), StringLiteral(value)));
    });
    pointCutConstructorArguments.positional.add(MapLiteral(sourceList));
    ...
    if (clz != null) {
      ...
      final List<MapLiteralEntry> fieldList = <MapLiteralEntry>[];
      final List<Field> fields = clz.fields;

      for (Field f in fields) {
        if (f.isConst && f.initializer is! BasicLiteral) {
          final entry = MapLiteralEntry(
          ...
          NamedExpression ne = NamedExpression(f.name.text, staticGet);
          fieldList.add(MapLiteralEntry(StringLiteral(f.name.text), ne.value));
        } else {
          final InstanceGet property = InstanceGet(InstanceAccessKind.Instance,
              thisE, Name(f.name.text, clz.parent as Library?),
      ...
      if (member is Procedure && member.isStatic) {
        ...
        pointCutConstructorArguments.positional.add(MapLiteral(fieldList));
      }
      ...
      for (Field f in fields) {
        ...
        if (f.isConst) {
          final ConstantExpression constantExpression =
              f.initializer! as ConstantExpression;
          filedsMap.add(
  ...
  static Arguments concatArguments4PointcutStubCall(
    ...
    for (VariableDeclaration variableDeclaration
      ...
      final DynamicInvocation methodInvocation = DynamicInvocation(
        ...
        LibraryManager.share.listGetProcedure!.name,
        getArguments,
      );
    ...
    for (VariableDeclaration variableDeclaration
      ...
      final DynamicInvocation methodInvocation = DynamicInvocation(
          ...
          LibraryManager.share.mapGetProcedure!.name,
          getArguments);
  ...
  static void insertProceedBranch(
    ...
    final Block block =
        LibraryManager.share.proceedProcedure!.function.body as Block;
    ...
    final Library core = LibraryManager.share.coreLib!;
  ...
  static bool checkIfSkipAOP(AopItemInfo aopItemInfo, Library curLibrary) {
    final Library aspectLibrary =
        aopItemInfo.aopMember!.parent!.parent as Library;
    Procedure? proceedProcedure = LibraryManager.share.proceedProcedure!;
    final Library pointCutPoint = proceedProcedure.parent!.parent as Library;
    return curLibrary == aspectLibrary || curLibrary == pointCutPoint;
  }

  static bool aspectEnable(Class cls) {
    bool enabled = false;
    List<Expression> annExprList = cls.annotations;
    for (Expression annExpr in annExprList) {
      //Release Mode
      if (annExpr is ConstantExpression) {
        final Constant constant = annExpr.constant;
        if (constant is InstanceConstant) {
          final CanonicalName? canonicalName =
              constant.classReference.canonicalName;
